<template>
    <div class=""></div>
    <div v-for="card in filteredData" 
        :key="card.id"
        @click="$emit('closeSearchResult', '.search-input')">
        <NuxtLink :to="`/${ card.media_type }/${ card.id }`">
            <div class="flex items-center flex-row w-full border-gray-400 border-solid border-b-[1px] pl-[10px] left-0 h-9 gap-2">
                ...
                <div class="flex items-center ">
                    {{ card.title || card.name }} ({{ card.media_type }})
                </div>
            </div>
        </NuxtLink>
    </div>
