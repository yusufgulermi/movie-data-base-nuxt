                ...
                <div class="p-[40px] flex gap-10 max-w-[1400px]">
                    ...
                    <div class="text-white">
                        <div class="flex flex-col py-[20px]">
                            <h1 class="text-[36px] font-bold">{{ fetchedData.list.title }}</h1>
                            <div class="flex w-full">
                                <span> {{ fetchedData.list.release_date }} •&nbsp;</span>
                                <ul class="flex">
                                    <li 
                                ...
                                <span>&nbsp;• {{ fetchedData.list.runtime }}m</span>
                            </div>
                        </div>
                        <div class="flex">
                        ...
                        <div class="relative h-[70px] flex items-center gap-[10px] mt-[30px]">
                            <div class="flex justify-center items-center w-[70px] h-[70px] bg-black rounded-[35px]">
                                <span class="text-white text-[14px]">{{ Number(fetchedData.list.vote_average?.toFixed(1) ?? 0) * 10 }}%</span>
            ...
            <div class="flex gap-[50px]">
                <div class="overflow-x-scroll w-[70%]">
                    <ul v-if="fetchedData.list.credits?.cast.length" class="flex relative w-max">
                        <li v-for="cast in fetchedData.list.credits?.cast.slice(0, 10)"
                            :key="cast?.cast_id">
                            <div v-if="cast?.profile_path"
                            class="flex flex-col h-[260px] w-[138px] m-[10px] rounded-lg overflow-hidden border-[1px] rounded-lg mb-2 shadow-lg">
                                <img class="w-[138px]"
                                    :src="`https://media.themoviedb.org/t/p/w138_and_h175_face${ cast?.profile_path }`">
                                <span class="font-bold pt-[10px] px-[10px]">{{ cast?.original_name }}</span>
                                <span class="px-[10px] text-[16px]">{{ cast?.character }}</span>
                            </div>
                        </li>
                    </ul>
                </div>
                <div class="w-[30%] flex flex-col items-start gap-[10px] justify-center">
                    ...
                    <div class="w-[50%] flex justify-center flex-col items-start border-[1px] rounded-lg shadow-lg px-[10px] py-[2px]">
                        ...
                        <span>{{ fetchedData.list.spoken_languages?.[0]?.english_name }}</span>
                    </div>
                    <div class="w-[50%] flex justify-center flex-col items-start border-[1px] rounded-lg shadow-lg px-[10px] py-[2px]">
                        <h1 class="font-bold">Budget</h1>
                        <span>{{ fetchedData.list.budget }}$</span>
                    </div>
                    <div class="w-[50%] flex justify-center flex-col items-start border-[1px] rounded-lg shadow-lg px-[10px] py-[2px]">
                        <h1 class="font-bold">Revenue</h1>
                        <span>{{ fetchedData.list.revenue }}$</span>
                    </div>
                </div>
            </div>
        </div>
...
<script setup>
    ...
    const { data } = await useFetch(`/api/product/movie/${ id }`, {
        getCachedData(key) {
